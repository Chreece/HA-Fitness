...
class LiveRuntime:
    ...
    async def async_initialize(self) -> None:
        ...
        if self.configured_transports:
            self.hass.async_create_task(self.async_ensure_hub_entry())
    ...
    async def async_ensure_hub_entry(self):
        """Ensure global Local Sensors has its own config-entry owner."""
        ...
        for entry in self.hass.config_entries.async_entries(DOMAIN):
            if entry.data.get("entry_type") == HUB_ENTRY_TYPE:
                ...
                return entry
        if not self.configured_transports:
            return None
        ...
        # The created entry may already be loaded by the time the flow returns.
        for entry in self.hass.config_entries.async_entries(DOMAIN):
            if entry.data.get("entry_type") == HUB_ENTRY_TYPE:
                self.hub_entry = entry
                return entry
