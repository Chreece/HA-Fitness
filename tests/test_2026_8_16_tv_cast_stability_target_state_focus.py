...
def test_cast_picker_does_not_guess_physical_tv_readiness_from_media_player_state():
    ...
    assert "l.start_tv_workout_preparing" in start
...
def test_remote_focus_visuals_have_clear_toolbar_spacing_and_modern_press_feedback():
    assert ':host([fitness-cast-receiver]) .tv-toolbar{grid-template-columns:auto minmax(76px,120px) auto minmax(180px,1fr);gap:5px;margin-bottom:11px' in FRONTEND
    ...
    assert 'scale(1.018)' in focus
    assert 'scale(.985)' in focus
    assert 'element.style.filter = "none"' in focus
