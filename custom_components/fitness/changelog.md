...
- **Cast card surface envelope follow-up** — Made DashCast and restricted Browser-TV cards paint their Home Assistant card surface on the outer receiver slot/custom-element host before any Shadow DOM boundary, so the full card background remains visible even when a TV WebView drops an inner `ha-card` paint layer; card-specific gradients and nested surfaces remain layered above the opaque underlay. Cache-busted both receiver frontend paths as `cast-ui-151`.

- **Universal metric timeline + Browser-TV Cast parity** — Made canonical profile sensors source-agnostic: every valid dated observation of the same metric competes only by timestamp, regardless of whether it came from a Fitness Test, direct device history, provider/entity, imported history or another adapter; source type is retained strictly as provenance, undated values remain fallback-only, and equivalent keys such as `vo2_max`/`vo2max` and `threshold_power`/`ftp_running` resolve onto one timeline. Fitness Test-derived canonical metrics are retained with their true completion time and linked study metadata without replacing any source history. Fixed restricted Browser-TV Cast detection so it now enters the exact same dense measured masonry, natural-size geometry, opaque card-surface and TV receiver behavior as DashCast instead of silently taking the normal dashboard branch. Frontend cache contract is now `cast-ui-150`.

- **DashCast dense card surfaces + canonical Fitness Test metrics** — Restored natural-size dense masonry for DashCast instead of the v148 wrapping-flex rows, removed the phantom legacy Cast scale from card-height geometry, and guaranteed an opaque theme card surface inside every mounted Cast card shadow root while preserving supported card gradients. Standardized Fitness Test outputs now publish immutable dated observations into the canonical profile resolver: a newer test-derived VO₂max can become the user VO₂max sensor/evaluation value without deleting Garmin/device/config history, a later device observation can become current again, and test-derived FTP feeds threshold-power/power-to-weight calculations. Fitness Test sensor/evaluation provenance identifies the exact test, result metric, method, completion time and linked scientific study; longitudinal VO₂max/threshold-power history retains the dated test observation. Frontend cache contract is now `cast-ui-149`.

- **DashCast singleton handoff + evidence-backed Fitness Test results** — Fixed the v147/v146 DashCast/Browser-TV bootstrap collision where an already-reserved Cast attempt re-reserved the same profile and surfaced as an HTTP 500 or exited after force-loading; receiver bootstrap now completes the matching server/Browser-TV reservation and reports real conflicts as HTTP 409. Unified profile Cast controls around one idle/connecting/connected state so every launch method is disabled while connecting, every method becomes Disconnect Cast only after receiver heartbeat, and failed/no-heartbeat attempts return to idle. Grouped built-in Fitness Tests by sport, linked every test title to its validation/source study, added 5-minute running, 3-minute all-out cycling and Rockport 1-mile walking protocols, and persist/display automatic test results where Fitness has measurable sensor evidence while explicitly marking manually scored strength tests as requiring a manual result. Frontend cache contract is now `cast-ui-148`.

- **Browser-TV Cast lifecycle, universal singleton and account identity hardening** — Added receiver-polled media commands so restricted Browser-TV Casts obey Play/Pause/Stop, made dashboard Stop Cast terminate Browser-TV receivers and page/tab exit release audio immediately, enforced one active-or-connecting Cast transport per profile including Browser-TV launch reservations, synchronized dashboard Cast controls with the universal receiver state, fixed Cast pointer focus and restricted config-flow rendering, replaced transformed absolute Cast card placement with centered natural-flow cards so hidden cards reserve no space and card/map layers retain their surfaces, and made Fitness usernames mandatory/unique with remote hostname/subdomain derived from the same single username. Frontend cache contract is now `cast-ui-147`.
...
- v123: stop already-installed verified-serial Bluetooth devices such as rotating-address CYCPLUS M1 units from reopening Add flows by merging the newly verified route into its accepted physical owner before discovery; localize all native Wellness device sensor names across all 15 bundled languages; and make the TV Cast exit contract literal: while the “press Back again to exit” notice is visible, the next distinct physical Back press exits Cast.
- Added profile-language-specific AI writing guidance for all AI calls while keeping one shared training/science prompt and stable machine-control tokens; tightened daily/weekly rationale length for cleaner cards.
- Hardened TV media transport with ordered command handling and generation cancellation so stale async RadioBrowser/provider Play operations cannot restart after Pause or create overlapping playback; Cast double-Back now supports Android/Google TV runtimes where the confirming Back arrives as a guarded history event.
- Removed forced bottom-scroll rewrites from desktop masonry, stabilized scrollbar geometry, increased AI-card resize hysteresis and debounce to eliminate the remaining end-of-dashboard AI-card jitter.
...
- Repaired the profile-language rollout and synchronized frontend cache contracts from the v116 base. Today’s AI text field and compact actions now sit directly below the suggestion title; today’s daily plan is the canonical day-0 decision used by the rolling 7-day plan; Workout Browser route extraction accepts canonical, namespaced, JSON and GeoJSON line payloads; and AI planning continues to use one language-neutral control prompt with a mandatory profile-language output instruction instead of maintaining divergent prompts per language.

- Fixed Fitness remote-language authority: a language selected on a successful Fitness portal login is persisted to the bound profile, and that profile language is now the single source of truth for dashboard translations, AI and TTS. Changing the language later in Fitness profile settings takes effect without a stale portal-session override.

- Added a persistent dashboard Fitness notification rail below the top toolbar. It keeps the newest active notification visible, remains visible when the toolbar auto-hides, supports browsing all active notices, moves shared-scale confirmation into the rail, can offer genuinely unassigned local Fitness sensors with Pair/Ignore actions, and temporarily gives structured-workout step instructions priority for at least ten seconds (or until the next step) before restoring the underlying notification. AI and Fitness Test prescriptions already use the same guided-step TTS pipeline; the visible step rail now mirrors those spoken instructions.
...
- Added first-install protocol/profile choices plus protocol adapter controls, including bounded manual Scan now actions that reuse the existing Bluetooth/ANT+ discovery managers instead of creating new pollers.
- Added optional dashboard composition settings, existing-entity RSS/music/TTS/lights/video/weather modules, per-profile Home Assistant theme selection, a read-only/no-extra-AI training coach card, a bounded body-weight/BMI trend card, and a canonical workout browser with multi-delete and confirmed Empty all actions.
- Added an About settings page with the installed version and bundled Unreleased changelog, and repaired historical physical-device titles that were stuck at the exact placeholder `Fitness sensor` without overwriting real user renames.

- Refined shared-scale routing: non-finite/unknown/unavailable values and exact repeat measurements are ignored across restarts; single-profile scales do not prompt when the reading already equals that profile's confirmed weight; every assigned user receives a private Yes/No question while administrators get the only user-selection control, and one user's No no longer hides the measurement from other users or administrators.
- Reworked mobile per-user Fitness settings to use the available dynamic viewport with a compact header, scrollable instructions and a reachable sticky action area so Main menu, Save and Close stay usable on small phones.
- Added a generic local-Bluetooth Device Registry connection identity and startup cleanup path so already-restored live/archive surfaces can converge while the physical device is offline, without adding any radio polling or vendor-specific hot-path logic.

- Improved dashboard settings UX: every background-backed settings transition now shows immediate progress, Main menu discards unsaved form edits instead of implicitly saving them, and flow Save buttons use normal content-sized controls.
- Centered the Home Assistant account assignment text across the complete selector control.
- Hardened physical-device reconciliation so restored CYCPLUS/local archive aliases with the same exact BLE route are merged and stale duplicate Home Assistant devices are removed deterministically.

- Added shared household scales without turning weight into an exclusive per-profile entity: each profile keeps an editable manual/current weight plus an optional shared scale sensor, one bounded event-driven router watches each configured scale only once, fresh measurements are matched to the nearest confirmed profile weight but are never applied automatically, and the profile dashboard asks the user to confirm/change the person or ignore the reading.
- Restored Fitness TV physical double-Back exit while retaining startup/native-picker/text-entry/held-key protections, and replaced the lingering blue remote-focus outline with a neutral Cast-only selection treatment.
- Added localized background-work feedback to profile Fitness settings so saved selections that trigger discovery/reconciliation clearly say that changes may take a moment to appear, without adding another polling loop.
- Centered the Home Assistant account-assignment selector text in the Fitness overview/profile controls.
- Replaced the vendor-specific Garmin setup menu with **Smart workout devices**: automatic bounded discovery lists physical workout-archive devices plus vendor setup guides, establishes one primary profile owner for stored-workout imports, keeps live sensor assignment independent, preserves ownership across strong-identity merges without silent cross-profile transfer, and treats vendor/device type/model as control-plane display/setup metadata only. Garmin remains capability-driven and model-independent, blank-name archive devices can be offered from verified protocol evidence, and the new flow reuses the existing scan cooldown/timeouts without adding permanent polling or GATT work.
- Added read-only direct local Garmin workout synchronization with model-independent vendor/protocol discovery, a bounded on-demand setup-guide scan, one-time bounded HA Bluetooth cache replay, dynamic V2 Multi-Link channel discovery and capability-driven V2 -> V1 -> V0 handshake fallback. Historical FIT import and strength-set normalization use hard discovery/connection/negotiation/session/import timeouts, wire-sized GFDI/COBS ceilings, bounded queues/files/catalogs/cache, small resumable batches, off-loop FIT inflate/decode, per-file checkpoints, Bluetooth ownership locks and sparse degraded/pairing/unsupported-MLR backoff. Automatic sync uses one tracked 30-minute timer, sparse unreachable checks, five-minute live-BLE deferral and HA advertisement-history clearing after GATT cleanup, so stable advertisements remain reliable without turning a missing/busy Garmin into a polling hot loop.
