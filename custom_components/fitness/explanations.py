...
_METHODS = {
    ...
    "sleep_consistency": ("Sleep timing variability", "standard deviation of merged sleep timing across the recent personal window", ["merged_sleep_start", "merged_sleep_end"], "use_baseline"),
    "sleep_deficit_7d": ("AASM/SRS adult sleep-duration reference", "sum(max(0, 7 h − observed nightly sleep)) across observed nights in the previous 7 days", ["merged_sleep_duration", "age"], "use_baseline"),
    "autonomic_recovery_trend": ("Personal HRV/RHR baseline", "latest nightly HRV relative to the 28-day mean, with resting-HR context", ["nightly_hrv", "resting_hr_history"], "use_baseline"),
    "cardiorespiratory_fitness_trend": ("Longitudinal VO₂max trend", "linear trend of measured/provider VO₂max daily statistics, normalized as % per 30 days", ["provider_vo2max_history"], "use_baseline"),
    "training_adaptation_status": ("Multisignal training-adaptation classification", "rule-based synthesis of recent-to-baseline load, VO₂max trend, HRV, resting HR and readiness; monitoring only, not diagnostic", ["completed_workouts", "banister_trimp", "vo2max_history", "hrv_history", "resting_hr_history", "readiness"], "use_baseline"),
    "training_load": ("Banister TRIMP longitudinal load", "7-day TRIMP compared with the weekly-equivalent TRIMP of the previous 28-day window", ["completed_workouts", "banister_trimp"], "use_load"),
    "heart_rate_recovery": ("Post-exercise heart-rate recovery", "exercise-end heart rate minus heart rate measured during recovery", ["exercise_end_hr", "post_exercise_hr"], "use_recovery"),
    "training_recovery_relationship": ("Personal training–sleep association", "Pearson correlation between workout TRIMP and the next sleep duration/HRV; descriptive, not causal", ["workout_trimp", "next_sleep_duration", "next_sleep_hrv"], "use_baseline"),
