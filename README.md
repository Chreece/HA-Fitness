...
<img src="assets/fitness-icon.png" alt="Fitness icon" width="150">

# Fitness for Home Assistant

**Bring your workouts, sleep, fitness sensors and long-term history together in Home Assistant.**
...
**Fitness turns Home Assistant into a local-first fitness hub for each person.**

It brings together **live exercise sensors, completed workouts, sleep and recovery data, and long-term Home Assistant history** from supported sources. When several devices describe the same workout or sleep session, Fitness can combine complementary data into one canonical record instead of treating every source as a separate activity.

Your existing devices and integrations remain the source of the measurements. **Fitness is the layer that discovers compatible data, normalizes it, merges it, calculates useful metrics and turns it into a consistent fitness picture inside Home Assistant.**

With Fitness you can:

- 🏃 **Follow workouts live** — heart rate, power, cadence, speed, pace, distance and derived intensity metrics.
- 📊 **Combine completed workouts** — normalize supported providers and merge complementary measurements belonging to the same activity.
- 😴 **Bring sleep sources together** — merge compatible sleep and recovery observations without duplicating the same night.
- 📈 **Understand long-term trends** — compare recent data with your own history and personal baselines.
- 🔬 **See how calculated metrics were produced** — Fitness-calculated entities expose their formula, data used, explanation and scientific basis where a specific evidence base applies.
- 🤖 **Use optional AI evaluation** — AI can turn the structured results into a readable summary in your configured language; it does not replace the deterministic calculations.
- 🏠 **Automate with the results** — use Fitness entities in Home Assistant dashboards, notifications, TTS, lights and automations.

> [!IMPORTANT]
> Fitness is a training and wellness integration, **not a medical device**. Its calculations and evaluations must not be interpreted as diagnosis or medical advice.

## How it fits together

![Fitness overview](assets/fitness-overview.png)

Fitness keeps the different layers deliberately separate:

1. **Live workout** collects direct exercise measurements and calculates useful real-time metrics.
2. **Workouts** normalizes and merges completed activities from live capture and supported providers.
3. **Sleep** normalizes and merges compatible sleep/recovery observations.
4. **History** builds personal context from workouts, sleep and Home Assistant Recorder data.
5. **Evaluation** uses the available evidence to create transparent, deterministic fitness insights and optional AI summaries.

This separation matters: a value coming directly from a sensor stays a provider measurement, while a value calculated by Fitness is identified as such and can expose the formula and inputs used.

## Your Fitness devices

Each configured person is organized into four localized Home Assistant devices:

| Device | Purpose |
|---|---|
| **Live workout** | The current workout assembled from supported live exercise sensors, plus real-time calculated metrics |
| **Workouts** | The latest completed workout and normalized/merged workout data |
| **Sleep** | The latest merged sleep and recovery data |
| **Evaluation** | Evidence-based long-term fitness, training and recovery context, plus optional AI evaluation |

Device and entity names follow the configured Fitness language. Fitness avoids repeating the integration/profile name unnecessarily in generated names.

Optional sensors use **lazy creation**: if Fitness has never had enough data to calculate a metric, that entity is not created. Once created, it remains registered and becomes unavailable when its prerequisites are temporarily missing. This keeps dashboards, automations and Recorder history stable.

## What Fitness evaluates

Fitness is designed to answer practical questions rather than simply mirror raw sensor values:
...
- Are VO₂max, resting heart rate, HRV, sleep and threshold-related trends changing over time?
- Which exact measurements and formulas produced a Fitness-calculated result?

The integration only creates an evaluation when its required data exists. Missing inputs are not fabricated.

## Local-first, multi-provider and transparent

**Local-first.** Home Assistant data stays local unless you explicitly configure an external AI provider or another external service.

**Multi-provider.** A workout does not have to come from one device. For example, heart rate may come from an ANT+ chest strap while power, cadence or speed come from another supported source. Completed-workout and sleep records can likewise be enriched with complementary data from multiple supported providers.

**Transparent calculations.** Direct provider values remain direct provider values. Fitness-calculated metrics can expose the formula, source entities and values used, a localized explanation, practical usefulness, and a scientific reference when the calculation has a specific evidence base.

**AI is optional.** Measurement collection, source selection, merging, unit conversion, statistics, formulas and deterministic evaluations work without AI. AI is used only to interpret the structured fitness context in more natural language.
...
Select compatible fitness devices that expose changing exercise data such as:
...
Setup only suggests devices whose capabilities look fitness-relevant. A device exposing only generic speed or distance is not enough by itself, which avoids accidentally suggesting radar, room-presence or appliance sensors. Heart-rate-only devices remain valid fitness sources.

If no live device is selected, Fitness can automatically discover supported ANT+ live sources.

Fitness-owned devices are excluded from source selection so the integration cannot accidentally feed its own generated entities back into discovery. When multiple candidate entities provide the same live metric, Fitness prefers an available numeric source rather than becoming stuck on an unavailable one.
...
When multiple providers describe the same completed workout, Fitness conservatively identifies duplicates and merges complementary fields into one canonical workout while preserving source provenance.
...
Fitness can consume supported sleep/recovery devices, including provider integrations and Sleep as Android-style entities when their sleep capabilities can actually be parsed. Setup suggests only compatible devices.

Records that represent the same night are merged conservatively into one canonical sleep record, so complementary duration, stage, HRV, heart-rate, respiratory, SpO₂ and provider-score fields can coexist without duplicate nights. Naps and clearly separate sleep periods remain separate.
...
Fitness uses calendar-style releases:
...
Development prereleases may append a stage such as `-beta.N`, but prerelease history is maintained in `CHANGELOG.md` rather than duplicated throughout this README.
