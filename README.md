...
#
### Scientific live-workout provenance

Fitness distinguishes direct live sensor measurements from values it calculates itself. Calculated live metrics expose transparent attributes with the formula, exact source data/profile values used, a localized explanation and practical interpretation, and a scientific citation where the calculation has a specific evidence base. Raw heart-rate, power, cadence, speed, distance and altitude measurements remain direct provider values and are not falsely labelled as Fitness calculations.

## Device organization

Each Fitness profile is organized into four concise, localized Home Assistant devices. The names follow the configured Fitness language and intentionally do not repeat the integration or profile name:

- **Evaluation** — evidence-based and AI-assisted interpretation.
- **Live workout** — the workout assembled in real time from supported live sensors.
- **Sleep** — merged sleep and recovery observations.
- **Workouts** — merged completed-workout history and derived workout metrics.

Existing device identifiers and entity unique IDs are unchanged, so this naming cleanup does not break history, dashboards or automations.
...
Fitness-owned Evaluation sensors expose **metric-specific evidence and provenance**: the relevant time windows, sample counts, baselines, comparison values, research reference and evidence level. Raw Live, Workout and Sleep entities stay intentionally compact. These attributes are deterministic and are never generated by AI.
...
## Supported workout and sleep sources

Fitness reads **Home Assistant entities** from source integrations; it does not log directly into your wearable account. Configure the source integration first, then select its device in Fitness.

### Completed-workout adapters

| Source | Home Assistant domain | Fitness adapter |
|---|---|---|
| Garmin Connect | `garmin_connect` | Dedicated |
| Strava | `ha_strava`, `strava` | Dedicated |
| Polar | `polar` | Dedicated |
| Hevy | `hevy` | Dedicated |
| Peloton | `peloton` | Dedicated |
| Oura | `oura` | Dedicated |
| WHOOP | `whoop` | Dedicated |
| Suunto | `suunto` | Provider-scoped normalized adapter |
| Fitbit | `fitbit` | Provider-scoped normalized adapter |
| Withings | `withings` | Provider-scoped normalized adapter |
| Other compatible integrations | varies | Conservative generic fallback |

Dedicated adapters understand provider-specific layouts. Provider-scoped normalized adapters own a known integration domain but only import recognizable activity/workout fields; they do not guess missing values. The generic fallback is available for other selected integrations when their entities expose a sufficiently clear workout contract.

### Sleep adapters

Every explicitly supported sleep provider now has a **separate adapter module and provider contract**:

| Source | Home Assistant domain | Recognized sleep/recovery data when exposed |
|---|---|---|
| Garmin Connect | `garmin_connect` | Duration, awake/light/deep/REM, score, HRV, sleep need, bedtime/wake time |
| Oura | `oura` | Duration, time in bed, stages, latency, score, efficiency, sleep HR/HRV, readiness, timing |
| Fitbit | `fitbit` | Minutes asleep/awake, time in bed, latency, efficiency, awakenings |
| Withings | `withings` | Stages, awake/latency, score, sleep HR, respiration, wake count, in-bed state |
| WHOOP | `whoop` | Performance, efficiency, stages, need/debt, recovery, HRV, RHR, SpO₂, respiration |
| Suunto | `suunto` | Duration, deep/REM/awake, score, HRV, sleep HR, recovery |
| SleepIQ | `sleepiq` | Duration, score, average HR, HRV, respiration, in-bed state |
| Eight Sleep | `eight_sleep`, `eightsleep` | Duration, score, time in bed, stages, HR, HRV, respiration, bed presence |
| Sleep as Android | `sleep_as_android` | Duration, deep sleep, quality/score and sleep HR where the installed integration exposes them |
| Other compatible integrations | varies | Conservative generic parser requiring multiple recognizable sleep fields |

Support is **capability-based**: not every device/account/source integration exposes every field. Fitness only creates data it can actually derive from available entities and never fabricates a missing workout, sleep period, timestamp or sleep stage.
