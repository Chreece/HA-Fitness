    ...
    def readiness_evaluation(self) -> dict[str, Any]:
        ...
        if latest_workout and latest_workout.start:
            ...
            if start is not None:
                ...
                # Recovery improves continuously with elapsed time instead of
                # jumping at fixed 6/12/24/36/48-hour boundaries. The anchor
                # scores preserve the previous readiness scale while linear
                # interpolation makes the value evolve naturally during the day.
                recovery_curve = (
                    (0.0, 30.0),
                    (6.0, 45.0),
                    (12.0, 65.0),
                ...
                training_score = recovery_curve[-1][1]
                for (start_h, start_score), (end_h, end_score) in zip(
                    recovery_curve,
                    recovery_curve[1:],
                    strict=False,
                ):
                    if hours_since <= end_h:
                        span_h = max(end_h - start_h, 0.001)
                        fraction = max(
                            0.0,
                            min(1.0, (hours_since - start_h) / span_h),
                        )
                        training_score = (
                            start_score
                            + fraction * (end_score - start_score)
                        )
                        break
