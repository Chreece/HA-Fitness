...
class BluetoothFitnessProvider:
    ...
    def _async_discovered(self, info, _change) -> None:
        ...
        sensor_id = f"bluetooth:{info.address.upper()}"
        self.runtime.register_sensor(
            LiveSensor(
                ...
                available=True,
                metadata={"service_uuids": sorted(uuids)},
            )
        )
        self._refresh_available()
    ...
    async def async_connect_profile(
        ...
        for sensor in sensors:
            if sensor.transport != self.transport or not sensor.address:
                continue
            ...
            ble_device = bluetooth.async_ble_device_from_address(
                self.hass, sensor.address, connectable=True
            )
            ...
            try:
                client = await establish_connection(
                    BleakClient,
                    device=ble_device,
                    name=sensor.name or sensor.address,
                    max_attempts=4,
                )
        ...
        def notify(parser):
            def _callback(_char, data):
                try:
                    ...
                    return
                if values:
                    self.runtime.publish(sensor.sensor_id, values)
            return _callback
