...
- Added read-only direct local Garmin workout synchronization with capability-selected GFDI V0/V1 or V2 Multi-Link backends, historical FIT import, strength-set normalization, a translated setup guide and manual retry control. The adapter uses hard connection/session/stage timeouts, bounded queues/files/catalogs/cache, small resumable batches, off-loop FIT inflate/decode, per-file checkpoints, Bluetooth ownership locks and sparse backoff for pairing/unsupported MLR so a busy or malformed wearable cannot stall Home Assistant.
